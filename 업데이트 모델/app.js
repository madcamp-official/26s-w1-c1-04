...
function render() {
  ...
  // 문/계단 — 이웃 (FR-002 / FR-006)
  const neighbors = Game.getNeighbors();
  el.doors.innerHTML =
    `<h2 class="doors-title">🚪 문 · 🪜 계단</h2>` +
...
function m10Evaluate() {
  ...
  if (m10LastKind() === "op" || m10LastKind() === "lp") return null; // 끝이 연산자/여는괄호
...
function updateM10() {
  el.m10Expr.innerHTML = m10.tokens.length
    ...
    : `<span class="m10-ph">숫자 타일과 연산자를 눌러 식을 만드세요</span>`;
  const v = m10Evaluate();
