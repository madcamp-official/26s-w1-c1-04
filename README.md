...
> 이 저장소의 **`main`은 소개용 랜딩 브랜치**입니다. 실제 게임 소스·문서 전체는 **[`terry2549` 브랜치](https://github.com/madcamp-official/26s-w1-c1-04/tree/terry2549)** 에 있습니다.
...
> 스토리 방향, 퍼즐 검수, 협업 규칙은 두 사람이 함께 정했습니다.

---

## 🎮 게임 소개

플레이어는 잠긴 건물에서 깨어난다. 모든 시계는 **03:14** 에 멈춰 있고, 어디에도 사람은 없다.
자신을 **가둔 누군가가 있다고 믿으며** 탈출을 시도하지만 —
정작 문을 여는 열쇠는 방마다 흩어진 **'기억의 조각'** 이다.

> **공포는 존재가 아니라 증거에서 온다.**
> 괴물·귀신·점프 스케어 대신, 반복되는 시간과 모순된 기억, 그리고 *플레이어 자신이 남긴 흔적*으로 불안을 만든다.
> 결말은 설명하지 않는다. 모든 단서를 연결해 **플레이어가 스스로 해석**한다.

- **퍼즐의 보상은 열쇠가 아니라 '새로운 사실'** 이다. 한 방에서 알게 된 사실이 다른 방 퍼즐의 실마리가 된다.
- 건물 전체가 하나의 거대한 퍼즐이 되도록 설계했다.
...
각 방의 오브젝트에 상호작용하면 퍼즐 모달이 열린다. 정답을 맞히면 **문이 열리거나 기억 조각/아이템**을 얻는다.
실제 SYL **'방탈출고사'** 유형을 리메이크했으며, 대표 예시는 다음과 같다.

| 층 | 방 | 퍼즐 | 유형(핵심 아하) | 정답 |
|---|---|---|---|---|
| 1F | 화장실 | 세면대 거울 | 김 서린 거울의 좌우 반전 | `202` |
| 1F | 104 | 고장난 계수기 | 회색 숫자 = 로마 숫자 삽입 (F·IV·E=FIVE) | `VIVA` |
| 1F | 105 | 세 개의 원 | 자모 영역 해독 | `바구니` |
| 2F | 교수연구실 | 액자 속 낱말 | 알파벳 등장 횟수 | `WHEN` |
| 2F | 서버실 | 세 방의 단서 | 조각 조합 + 크로스워드 | `ADOPT` |
| 옥상 | — | 기억 조각 배치 | 조각 5개 → 시:분분:초초 | `3 : 14 : 15` |
...
전체 소스는 **[`terry2549` 브랜치](https://github.com/madcamp-official/26s-w1-c1-04/tree/terry2549)** 에 있습니다.

```bash
# 1) 클론 후 terry2549 브랜치 체크아웃
git clone https://github.com/madcamp-official/26s-w1-c1-04.git
cd 26s-w1-c1-04
git checkout terry2549
...
> 게임만 확인하려면 `업데이트 모델/move2d_update.html` 을 브라우저로 바로 열어도 됩니다(리더보드는 서버가 떠 있어야 저장·조회).
...
## 📁 프로젝트 구조 (`terry2549` 브랜치)

```
26s-w1-c1-04/
├─ 업데이트 모델/            ← 실행 대상(최신)
│  ├─ move2d_update.html    ← 게임 본체(단일 파일: HTML+CSS+JS+Canvas, 이미지 base64 내장)
│  ├─ server.js             ← Express 정적 서빙 + 리더보드 API
│  ├─ scores.json           ← 리더보드 데이터
│  └─ package.json          ← 의존성(express)·start 스크립트
├─ background/              ← 퍼즐 원본 이미지 · 풀이 문서
├─ 문서들/                   ← 스토리·연출·기능 명세 등 설계 문서
└─ docs/                    ← 산출물 문서 7종(기획·기능·IA·DB·API·배포·회고)
```
